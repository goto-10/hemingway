...
class Converter(object):
  ...
  def convert_source(self, source):
    ...
    body = "".join(results)
    return _HTML_TEMPLATE % (self.context.get_headers(), body)
...
def please_run_the_tests():
  ...
  class HemingwayTest(unittest.TestCase):
    ...
    def test_strip_initial(self):
      def run_test(expected, initial, line):
        found = CommentBlock.strip_initial(initial, line)
        self.assertEquals(expected, found)
      run_test("Foo", "#", "#Foo")
      run_test("Foo", "#", "# Foo")
      run_test(" Foo", "#", "#  Foo")
